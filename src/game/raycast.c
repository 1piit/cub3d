...
/*   Updated: 2026/03/31 16:21:13 by ptricaud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*raycast_game(void *arg)
{
    ...
    while (1)
	{
        ...
        while (i < t_data_ptr->end_x)
        {
			...
			t_data_ptr->th_player.ray_dir.x = t_data_ptr->th_player.dir_x
				+ t_data_ptr->th_player.plane.plane_x
				* t_data_ptr->th_player.camera_x;
			t_data_ptr->th_player.ray_dir.y = t_data_ptr->th_player.dir_y
				+ t_data_ptr->th_player.plane.plane_y
				* t_data_ptr->th_player.camera_x;
			ft_dda(data, t_data_ptr, t_data_ptr->th_player.ray_dir, &box);
			if (box.perp_wd <= 0)
			{
				i++;
				continue ;
			}
			ray.line_height = (int)(data->game.win_height / box.perp_wd);
            ...
            if (ray.draw_start < 0)
                ray.draw_start = 0;
			draw_wall(t_data_ptr, data, ray, i, &box);
            i++;
        }
        pthread_barrier_wait(&data->barrier_start);
    }
...
void	draw_vlines(t_data *data, int i, int drawS, int drawE, int color)
{
	...
	while (drawS != drawE)
	{
		put_pixel(&data->game.game_img, i, drawS, color);
		drawS++;
	}
}
	...
	while (1)
	{
		...
		while (i < t_data_ptr->end_x)
		{
			...
			t_data_ptr->th_player.ray_dir.y = t_data_ptr->th_player.dir_y
				+ t_data_ptr->th_player.plane.plane_y
				* t_data_ptr->th_player.camera_x;
			ft_dda(data, t_data_ptr->th_player.ray_dir);
			lineHeight = (int)(data->game.win_height / data->game.box.perp_wd) * 1.3;
			...
			if (data->game.box.side == 0)
				wall_x = t_data_ptr->th_player.pos_y + data->game.box.perp_wd
					* t_data_ptr->th_player.ray_dir.y;
			else
				wall_x = t_data_ptr->th_player.pos_x + data->game.box.perp_wd
					* t_data_ptr->th_player.ray_dir.x;
			wall_x -= floor(wall_x);
			draw_wall(data, drawStart, drawEnd, lineHeight, i, wall_x);
			i++;
		}
